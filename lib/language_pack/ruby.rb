...
ERROR
  ...
  def install_node
    log("node") do
      ...
      result = run "curl #{NODE_JS_BASE_URL}/v#{NODE_JS_VERSION}/node-v#{NODE_JS_VERSION}-linux-x64.tar.gz -s -o - | tar xzf - -C /tmp && mv /tmp/node-v#{NODE_JS_VERSION}-linux-x64/* bin/node && chmod +x bin/node/bin/* && PATH=#{HEROKU_DIR}/bin/node/bin/:$PATH"
      puts result
