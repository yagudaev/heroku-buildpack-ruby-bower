...
ERROR
  ...
  def install_node
    log("node") do
      ...
      result = run "curl #{NODE_JS_BASE_URL}/v#{NODE_JS_VERSION}/node-v#{NODE_JS_VERSION}-linux-x64.tar.gz -s -o - | tar xzf - -C /tmp && mkdir -p build/node && mv /tmp/node-v#{NODE_JS_VERSION}-linux-x64/* build/node && chmod +x build/node/bin/* && ln -s build/node/bin/node bin/node && ln -s build/node/bin/npm bin/npm"
      puts result
